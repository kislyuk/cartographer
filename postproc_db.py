...
'''
        futures.append(pool.submit(compute_occlusions((lng, lat), (lng+step, lat+step))
        futures = []
        points = list(POI.objects(at__geo_within_box=((lng, lat), (lng+step, lat+step))))
        for i, p1 in enumerate(points):
            for j, p2 in enumerate(points[i+1:]):
                coords1, coords2 = p1.at['coordinates'], p2.at['coordinates']
                dist = math.sqrt(abs(coords1[0]-coords2[0])**2 + abs(coords1[1]-coords2[1])**2)
